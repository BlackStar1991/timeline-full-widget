        ...
        <div class="timeline-wrapper">
            ...
            <ul class="timeline timeline-animation-marker">
                ...
                <ul class="timeline">
                    ...
                    var bg_color = '';
                    if ( item.li_bg_color ) {
                    bg_color = 'background-color:' + _.escape( item.li_bg_color ) + ';';
                    }

                    var showMarker = settings.tl_show_marker === 'yes';

                    // dynamic tag and safe title html
                    var tag = settings.header_tag ? settings.header_tag : 'h2';
                    function buildLinkHtml( item ) {
                    if ( item.link_url && item.link_url.url ) {
                    var href_url = _.escape( item.link_url.url );
                    var attrs = ' href="' + href_url + '"';
                    if ( item.link_url.is_external ) attrs += ' target="_blank"';
                    if ( item.link_url.nofollow ) attrs += ' rel="nofollow"';
                    return '<a' + attrs + '>' + _.escape( item.list_title ) + '</a>';
                    }
                    return _.escape( item.list_title );
                    }
                    var titleInner = buildLinkHtml( item );
                    var titleHtml = '<' + tag + ' class="' + _.escape( titleClass ) + '">' + titleInner + '</' + tag + '>';
                #>
                    ...
                    <div class="timeline-panel" style="{{ bg_color }}">
                        <div class="tl-content">
                            ...
                            <div class="timeline_pic pull-left">
                                ...
                                <# } else { #>
                                <img src="{{{ image_url }}}"
                                     alt="{{ item.media_image && item.media_image.alt ? item.media_image.alt : ( item.media_image && item.media_image.title ? item.media_image.title : '' ) }}"/>
                                <# } #>
                            </div>
                            <# } #>
