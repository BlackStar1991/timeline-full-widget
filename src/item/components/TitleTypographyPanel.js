...
export default function TitleTypographyPanel({ attrs = {}, setAttributes }) {
    const { titleFontSize, titleFontWeight, titleMarginTop, titleMarginBottom, titleLineHeight, titleFontFamily } = attrs;

    return (
        <PanelBody title={__('Title Typography', 'timeline-full-widget')} initialOpen={true}>
            <FontSizePicker
                fontSizes={[
                ...
                onChange={(newSize) => {
                    if ( newSize === undefined ) {
                        setAttributes({ titleFontSize: '22', titleFontUnit: 'px' });
                        return;
                    }
                    setAttributes({ titleFontSize: String(newSize), titleFontUnit: 'px' });
                }}
                withSlider
            />
            ...
            <FontFamilySelect value={ titleFontFamily } onChange={(val) => setAttributes({ titleFontFamily: val })} />

            <RangeControl
                label={__('Margin Top (px)', 'timeline-full-widget')}
                value={ Number(titleMarginTop) || 0 }
                onChange={(value) => setAttributes({ titleMarginTop: String(value) })}
                min={0} max={100}
            />
            <RangeControl
            ...
            <RangeControl
                ...
                onChange={(value) => setAttributes({ titleLineHeight: String(value) })}
            />
        </PanelBody>
    );
}
