...
export function initTimelineAnimation(scopeEl) {
    ...
    const canUseParentCoords = !!(isInIframe && parentWindow && window.frameElement && typeof window.frameElement.getBoundingClientRect === 'function');
    ...
    const getIframeElementRect = () => {
        try {
            ...
            if (!fe || typeof fe.getBoundingClientRect !== 'function') return null;
    ...
    const getRootMiddle = () => {
        if (canUseParentCoords) {
            try { return parentWindow.innerHeight / 2; } catch (e) { /* fallback below */ }
        }
    ...
    function updateStuckByLine() {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            const center = ((itemRect.top + itemRect.bottom) / 2) + (canUseParentCoords ? iframeOffsetTop : 0);
    ...
    try {
        if (scrollParent === window) {
            ...
            if (isInIframe && parentWindow) {
                ...
                parentWindow.addEventListener('scroll', onScrollOrResize, { passive: true });
            ...
            if (isInIframe && parentWindow) {
                log('Adding scroll/resize listeners to parent window (custom scrollParent case)');
                parentWindow.addEventListener('scroll', onScrollOrResize, { passive: true });
                parentWindow.addEventListener('resize', onScrollOrResize);
                parentHandlersAdded.scroll = true;
                parentHandlersAdded.resize = true;
    ...
    console.debug('[ZA] animation.js initTimelineAnimation executed', { inIframe: isInIframe, canUseParentCoords });
    ...
    (function startIfVisible() {
        ...
        if (canUseParentCoords) {
            const r = rectInParent(rect);
            const inViewport = r.bottom > 0 && r.top < (parentWindow.innerHeight || parentWindow.document.documentElement.clientHeight);
        ...
        try {
            if (scrollParent === window) {
                ...
                if (isInIframe && parentWindow && parentHandlersAdded.scroll) {
                    ...
                    try { parentWindow.removeEventListener('scroll', onScrollOrResize, { passive: true }); } catch (e) { parentWindow.removeEventListener('scroll', onScrollOrResize); }
                    try { parentWindow.removeEventListener('resize', onScrollOrResize); } catch (e) {}
                }
            } else {
                ...
                if (isInIframe && parentWindow && parentHandlersAdded.scroll) {
                    log('Removing scroll/resize listeners from parent window (custom scrollParent case)');
                    try { parentWindow.removeEventListener('scroll', onScrollOrResize, { passive: true }); } catch (e) { parentWindow.removeEventListener('scroll', onScrollOrResize); }
                    try { parentWindow.removeEventListener('resize', onScrollOrResize); } catch (e) {}
                }
            }
    ...
    try {
        ...
        wrappers.forEach((wrapper, index) => {
            ...
            try {
                ...
                log('[ZA animation.js] Wrapper #' + index + ' initialized, destroy fn:', typeof destroy);
            } catch (e) {
                console.error('[za-timeline] initAllWidgets wrapper init failed', e);
            }
        });
    } catch (e) {
